...
6. 分析文本中的操作信息，包括：
   ...
   - 对话操作：如"张三对李四说"、"李四说"等
   - 姿态操作：如"张三坐下"、"李四站起来"、"王五挥手"、"赵六跳了起来"等
7. 对于移动操作，记录操作类型、执行角色、目标角色（如果有）、移动类型（如"走到身边"、"散步"、"离开"等）等信息
8. 对于姿态操作，使用 type: "gesture"，并记录 gesture_type（如"sit", "stand", "wave", "jump"）
9. 分析对话关系，确保对话之前两个角色是靠近的，如果需要，生成相应的移动操作
10. **重要：提取的对话内容（content）必须极度精简，直接还原文本中的原话，严禁自行扩展或润色。**
...
请以 JSON 格式返回，格式如下：
{
    ...
    "actions": [
        ...
        },
        {
            "type": "gesture",
            "character": "执行角色",
            "gesture_type": "动作类型（sit/stand/wave/jump）",
            "description": "动作描述"
        },
        {
            "type": "dialogue",
